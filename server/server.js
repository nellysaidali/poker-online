...
// IMPORTANT : mets ici TON FRONT (statique) + tes locals
...
const corsOptions = {
  origin: (origin, cb) => {
    // origin null => curl/postman / certaines requêtes
...
// Render aime bien polling+websocket (polling d’abord)
...
const rooms = {}; // <-- OBLIGATOIRE (si absent: ton jeu “marche” pas)

/* =====================
   Utils (deck / shuffle / eval)
===================== */
...
function showdown(game) {
  ...
  for (const s of stillIn) {
    const seven = [...s.hand, ...game.board].filter(Boolean);
    // sécurité : si pas 7 cartes, on “baisse” la force
    if (seven.length < 7) continue;
    ranks.set(s.seat, bestRank7(seven.slice(0, 7)));
  }
...
function preflopScore(hand) {
  ...
  if (hi >= 13 && lo >= 10) score += 6;
  if (hi >= 14 && lo >= 11) score += 4;
  return score;
}

function potOdds(need, pot) {
  if (need <= 0) return 0;
  return need / (pot + need);
}
...
function botDecide(game, seatIndex) {
  ...
  const aggro = 0.55 + Math.random() * 0.25;
...
/* =====================
   Socket handlers (le jeu)
===================== */

io.on("connection", (socket) => {
  socket.on("ping:test", (cb) => cb?.({ ok: true }));
